...
%% ME 3360 - Proj 2 - Part (e (Ramp Response))
%Define variables
l = 0.5;            % m
r = 1/6;            % m
N = 126;            % # coils
...
%Run simulation
sim Part_E_Ramp_Simulink;

%Plot motor torque, car velocity, and armature current for voltage ramp
figure(1);
subplot(3,1,1);
plot(Tm.Time, Tm.Data, 'b-', 'LineWidth', 1.5);
xlabel ('t [sec]');
ylabel ('T (t) [N-m]');
title('Motor Torque Response for Voltage Ramp');
subplot(3,1,2);
plot(xdot.Time, xdot.Data, 'b-', 'LineWidth', 1.5);
xlabel ('t [sec]');
ylabel ('xdot(t) [m/s]');
title('Car Velocity Response for Voltage Ramp');
subplot(3,1,3);
